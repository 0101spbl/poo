...
class Player
  ...
  # new player name and 10 life points
  def initialize (name)
    @name = name
    @life_points = 10
  end

  # display life level reached
  def show_state
    if @life_points <= 0
  ...
  def gets_damage(damage)
    ...
    if @life_points <= 0
      puts "\n"
      puts "Le jour #{@name} a été tué ! -----"
    end
end

  # attack from the current player to another 
  def attacks (player)
    ...
    # damage to the "player" in argument
    player.gets_damage(damage)
  end

  # random dice number in gets_damage function
  def compute_damage
    rand(1..6)
  end
end
...
class HumanPlayer < Player
  ...
  # current user life and weapon level display 
  def show_state
    if @life_points <= 0
      puts ">>> #{@name} a 0 points de vie"
    else
